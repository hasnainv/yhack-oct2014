...
_index = similarities.SparseMatrixSimilarity(tfidf[_corpus.vector], num_features=self.ndic)
	...
	def __init__(self):
		...
		for idea in self.ideas.find():
			...
			for word in line.split(' '):
				...
				if word in Dic.words.keys():
					num = Dic.words[word]
				else:
					Dic.words[word] = Dic.count
					num = Dic.count
					Dic.count += 1
